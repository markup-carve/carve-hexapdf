...
# pin. Three ways of measuring a different engine all left `bundle exec rake
# test` green and said nothing:
#
#   * the `ref:` is dropped or edited to something that no longer resolves, so
#     the bundle follows carve-rb's default branch - which is how #10 happened,
#     two AST renames landing upstream while the badge stayed green;
#   * `CARVE_RB_PATH` is set, which swaps the pinned engine for whatever a
#     sibling working tree is parked on (until this run it did not even need
#     setting: the directory merely existing was enough);
#   * a restored bundler cache resolved before the pin last moved.
#
# It matters more here than in a renderer that would crash on a surprise: this
# gem's own suite is 82 runs green against carve-lang 0.1.0, the engine the
# gemspec floor excludes, because most of it builds ASTs by hand. The engine can
# change what the PDFs look like without changing whether the suite passes -
# measured, `examples/01-spec.crv` renders to 3268 bytes on 0.1.1 and 3341 on
# 0.1.0.
#
